...
import { getDepartments, getUEs, getSubjects, createSubject, createDepartment, getTeachers, importTeachers } from '../api/auth';
import './Academia.css';
...
function DeptModal({ onClose, onSaved }) {
  ...
  return (
    <div className="modal-overlay" onClick={e => { if (e.target === e.currentTarget) onClose(); }}>
      <div className="modal">
        <div className="modal-header">
          <h2 className="modal-title">Nouveau département</h2>
          <button className="btn btn-ghost btn-icon" onClick={onClose}><span className="material-symbols-outlined" style={{ fontSize: 18 }}>close</span></button>
        </div>
        <form onSubmit={handle}>
          <div className="form-grid-2">
            <div className="form-field"><label className="form-label">Nom *</label><input className="form-input" placeholder="ex: Informatique" value={form.name} onChange={e => set('name', e.target.value)} /></div>
            <div className="form-field"><label className="form-label">Code</label><input className="form-input" placeholder="ex: INFO" value={form.code} onChange={e => set('code', e.target.value)} /></div>
          </div>
          <div className="form-field" style={{ marginTop: 12 }}>
            <label className="form-label">Description</label>
            <textarea className="form-input" rows={3} placeholder="Description facultative..." value={form.description} onChange={e => set('description', e.target.value)} />
          </div>
          <div className="modal-footer">
            <button type="button" className="btn btn-ghost" onClick={onClose}>Annuler</button>
            <button type="submit" className="btn btn-primary" disabled={saving}>
...
export default function Academia() {
  ...
  return (
    <Layout title="Gestion Académique" subtitle="Enseignants, départements, filières et matières">
        ...
        <div className="card" style={{ padding: 0, overflow: 'hidden' }}>
          <div style={{ padding: '14px 20px', borderBottom: '1px solid var(--border)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            ...
            <button className="btn btn-primary btn-sm" onClick={() => setShowDeptModal(true)}>
              <span className="material-symbols-outlined" style={{ fontSize: 14 }}>add</span>Nouveau département
            </button>
          </div>
          <div className="data-table-wrap">
            <table className="data-table">
      ...
      {showDeptModal && (
        <DeptModal onClose={() => setShowDeptModal(false)} onSaved={() => { setShowDeptModal(false); load(); }} />
      )}
    </Layout>
  );
}
