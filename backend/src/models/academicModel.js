...
export const createUE = async ({name,code,level,department_id,academic_year_id}) => (await pool.query(`INSERT INTO ues(name,code,level,department_id,academic_year_id)VALUES($1,$2,$3,$4,$5)RETURNING *`,[name,code||null,level,department_id||null,academic_year_id||null])).rows[0];
...
export const getAllSubjects = async (ue_id=null) => {
  let q=`SELECT s.*,u.name as ue_name,u.level FROM subjects s LEFT JOIN ues u ON u.id=s.ue_id WHERE 1=1`;
  const p=[];
  if(ue_id){q+=` AND s.ue_id=$1`;p.push(ue_id);}
  q+=` ORDER BY s.name`;
  return(await pool.query(q,p)).rows;
};
export const getSubjectById = async (id) => (await pool.query(`SELECT s.*,u.name as ue_name FROM subjects s LEFT JOIN ues u ON u.id=s.ue_id WHERE s.id=$1`,[id])).rows[0]||null;
